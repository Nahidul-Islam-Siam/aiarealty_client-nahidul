...
export default function MortgageList({ page }: { page?: string }) {
  ...
  const { data,isLoading ,refetch} = useGetPartnersQuery({
    role: "MORTGAGE", // Role filter only
  });
  ...
  const mortgages = data?.data?.data || []; // Access the "data" key in the response
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px]">
            <thead className="bg-white border-b border-gray-200">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500">Mortgage Name</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500">Mortgage ID</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {mortgages
                .filter((mortgage) => mortgage.name.toLowerCase().includes(searchTerm.toLowerCase()))
                .map((mortgage, idx) => (
