...
export default function DeveloperList   ({ page }: { page?: string }) {
 ...
 const [currentPage, setCurrentPage] = useState(1)
  ...
  const { data,  isLoading, refetch } = useGetPartnersQuery({
    role: "DEVELOPER", // Role filter only
  });
  ...
  const delevelopers = data?.data?.data || []; // Access the "data" key in the response
          ...
          <div className="max-w-3xl 2xl:max-w-xl mb-8">
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-3xl font-bold text-gray-900 mb-1">{delevelopers?.length}</div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px]">
            <thead className="bg-white border-b border-gray-200">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500">Developer Name</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500">Developer ID</th>
              ...
              {delevelopers
                ...
                .map((developer, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    <td className="px-4 py-4 text-sm text-gray-900">{developer?.name}</td>
                    <td className="px-4 py-4 text-sm text-gray-900">{developer?.id}</td>
                    <td className="px-4 py-4 text-sm text-gray-900">{developer.profile?.company || "N/A"}</td>
                    <td className="px-4 py-4 text-sm text-gray-900">{developer?.email}</td>
                    <td className="px-4 py-4 text-sm text-gray-900">{developer.profile?.phone || "N/A"}</td>
                    <td className="px-4 py-4 text-sm text-gray-900">{developer?.profile?.country || "N/A"}</td>
